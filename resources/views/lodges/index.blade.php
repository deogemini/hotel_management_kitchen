...
<tr><td>{{ $loop->iteration }}</td><td>{{ $lodge->name }}</td><td>{{ $lodge->location }}</td><td>{{ $lodge->phone_number }}</td><td>{{ $lodge->users_count }}</td><td>{{ $lodge->rooms_count }}</td><td><a class="btn btn-sm btn-info" href="{{ route('lodges.edit', $lodge) }}">Edit</a> <form method="POST" action="{{ route('lodges.destroy', $lodge) }}" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this lodge?');">@csrf @method('DELETE')<button type="submit" class="btn btn-sm btn-danger">Delete</button></form></td></tr>
@endforeach
</tbody></table></div></div>
@endsection
